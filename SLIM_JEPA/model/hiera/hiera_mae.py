...
class SlimEncoder(Hiera):
    """SLIM 4D Hiera 编码器,用于特征提取。
    键与 best_model.pth 的编码器部分一致:
      pos_embed_spatial / pos_embed_temporal / patch_embed.* / blocks.* / encoder_norm
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # best_model.pth 用 encoder_norm 而非 norm,且没有分类 head
        del self.head
        self.encoder_norm = self.norm
        del self.norm

    def load_from_mae(self, ckpt_path):
        """只加载编码器键(跳过 mask_token / decoder_* / multi_scale_*)"""
        import torch as _t
        ckpt = _t.load(ckpt_path, map_location='cpu', weights_only=False)
        sd = ckpt['model_state_dict']
        enc = {}
        for k, v in sd.items():
            if k.startswith(('mask_token', 'decoder', 'multi_scale')):
                continue
            enc[k] = v
        missing, unexpected = self.load_state_dict(enc, strict=False)
        print('编码器键加载完成. 缺失:', len(missing), ' 多余:', len(unexpected))
        if missing:
            print('缺失键(前10):', missing[:10])
        if unexpected:
            print('多余键(前10):', unexpected[:10])
        return missing

    def forward_features(self, x, mask=None):
        """编码器前向,返回池化特征 (B, C)。
        x: (B, C, T, D, H, W);  mask: (B, N_mu) 布尔, True=保留该mask unit
        """
        if mask is not None:
            pmask = mask.view(x.shape[0], 1, *self.mask_spatial_shape)
        else:
            pmask = None
        x = self.patch_embed(x, mask=pmask)
