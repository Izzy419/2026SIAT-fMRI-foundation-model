...
class Conv4d(nn.Module):
    """4D conv (kT,kD,kH,kW). SLIM uses kT=1: applies Conv3d over (D,H,W) per T frame."""
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        nn.init.kaiming_normal_(self.weight, a=1e-4)

    def forward(self, x):
        # x: (N, C, T, D, H, W)
        N, C, T, D, H, W = x.shape
        kT, kD, kH, kW = self.kernel_size
        s = self.stride if isinstance(self.stride, (tuple, list)) else (self.stride,) * 4
        pd = self.padding if isinstance(self.padding, (tuple, list)) else (self.padding,) * 4
        assert kT == 1 and s[0] == 1 and pd[0] == 0, "Conv4d only supports kT=1 for now (SLIM uses kT=1)"
        w = self.weight[:, :, 0]                      # (out, C, kD, kH, kW)
        x = x.permute(0, 2, 1, 3, 4, 5).reshape(N * T, C, D, H, W)
        out = F.conv3d(x, w, self.bias, stride=s[1:], padding=pd[1:])
        out = out.view(N, T, *out.shape[1:]).permute(0, 2, 1, 3, 4, 5)   # (N,out,T,D',H',W')
        return out
...
def conv_nd(n: int) -> Type[nn.Module]:
    ...
    """
    return [nn.Identity, nn.Conv1d, nn.Conv2d, nn.Conv3d, Conv4d][n]
...
def get_resized_mask(target_size: torch.Size, mask: torch.Tensor) -> torch.Tensor:
    ...
    if mask.shape[2:] != target_size:
        if mask.dim() == 6 and len(target_size) == 4:
            # 4D fMRI: 最近邻上采样, 把每个 mask unit 扩到它覆盖的区域
            ratios = tuple(t // m for t, m in zip(target_size, mask.shape[2:]))
            out = mask
            for dim, r in zip((2, 3, 4, 5), ratios):
                out = out.repeat_interleave(r, dim=dim)
            return out
